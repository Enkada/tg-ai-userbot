...
import { containsFakePhotoBlock, finalizeReply, parseToolCall, stripToolCalls } from './tools.js';
import {
  ackLine,
  isSelfieAvailable,
...
async function processMessage(msg: Message, senderId: number, selfName: string): Promise<void> {
  ...
  try {
    ...
    const reply = await withTyping(client, msg.chat, async () => {
      ...
      // ack reply below is sent and persisted. Only a *proper* parsed tool call opens the
      // image path — bracketed imitation text is not a call (tools have syntax); it's sent
      // verbatim and handled by the promise gate below instead.
      const call = parseToolCall(result.content);
      const selfieProse =
        isSelfieAvailable() && call?.name === 'send_selfie'
          ? String(call.arguments.prompt ?? '').trim()
          : '';
      // Strip any leftover tool call (cap hit) so no raw tag reaches the chat, then flush
      // the final bubble (or send the whole thing if nothing streamed).
    ...
    } else if (containsFakePhotoBlock(reply.replyText) || looksLikePhotoPromise(reply.replyText)) {
      // No proper call, but the reply either wrote a fake "[you sent a photo: …]" block or
      // reads like "gimme a sec, i'll send a pic" — run the promise gate: an LLM reread
      // whose output is discarded unless it parses as a *valid* send_selfie call. Text is
      // never treated as a tool call directly; the gate's redo is the only repair path.
      await maybeRepairPromise({
        client,
        peer: msg.chat,
        chatId,
        userName,
